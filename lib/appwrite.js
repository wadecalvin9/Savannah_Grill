...
export const createOrder = async ({
  ...
  items,
  totalPrice,
  // optional payment fields (can be filled later via setOrderPaymentDetails)
  paymentMethod = null,
  paymentStatus = null,
  amount = null,          // cents
  currency = 'KES',
  ...
  try {
    ...
    // Only write payment fields if they are provided
    if (paymentMethod) payload.payment_method = paymentMethod;
    if (paymentStatus) payload.payment_status = paymentStatus;
    if (amount != null) payload.amount = amount;
    if (currency) payload.currency = currency;
    if (paystackReference) payload.paystack_reference = paystackReference;
...
/**
 * Shared mapper – keeps every list function consistent
 */
const mapOrderDocument = (doc) => ({
  ...
  paystack_access_code: doc.paystack_access_code || null,
  amount: doc.amount ?? null,               // cents
  amountKes: doc.amount != null ? doc.amount / 100 : null,
  currency: doc.currency || 'KES',
  paid_at: doc.paid_at || null,
  payment_channel: doc.payment_channel || null,
...
export const getOrders = async () => {
    try {
        ...
        return res.documents.map(mapOrderDocument)
    } catch (error) {
...
export const getStaffOrders = async () => {
  try {
    ...
    .filter(doc => {
        if(!activeStatuses.includes(doc.status)) return false;

    // Hide unpaid or expired Paystack orders from kitchen
            if (doc.payment_method === 'paystack') {
                if (doc.payment_status === 'pending' || doc.payment_status === 'expired') {
                    return false;
                }
            }
            return true;
...
/** Convert KES → cents (Paystack smallest unit) */
export const kesToCents = (kes) => Math.round(Number(kes) * 100);

/** Convert cents → KES for display */
export const centsToKes = (cents) => Number(cents) / 100;

/** Unique reference for Paystack */
export const generatePaystackReference = (orderId) => {
  const ts = Date.now();
  const rand = Math.floor(Math.random() * 10000).toString().padStart(4, '0');
  return `SG-${orderId || 'ORD'}-${ts}-${rand}`;
};



/**
 * Set payment details on an order (call at checkout)
 */
export async function setOrderPaymentDetails(
  orderId,
  { paymentMethod, amountKes, currency = 'KES', paystackReference = null }
) {
...
/**
 * Mark order as paid (Paystack success or COD collected)
 */
export async function markOrderAsPaid(
  orderId,
  { paystackReference = null, paymentChannel = null, paidAt = new Date().toISOString() } = {}
) {
  if (!orderId) throw new Error('orderId is required');
  ...
  if (paystackReference) payload.paystack_reference = paystackReference;
  if (paymentChannel) payload.payment_channel = paymentChannel;
...
export async function markOrderPaymentFailed(orderId, paystackReference = null) {
  const payload = { payment_status: PAYMENT_STATUS.FAILED };
  if (paystackReference) payload.paystack_reference = paystackReference;
...
/** Rider confirms cash / M-Pesa collected on delivery */
export async function confirmCashCollected(orderId) {
  return markOrderAsPaid(orderId, { paymentChannel: 'cash_on_delivery' });
}
...
export function needsPaymentCollection(order) {
  return (
    order?.payment_method === PAYMENT_METHODS.CASH_ON_DELIVERY &&
    order?.payment_status === PAYMENT_STATUS.AWAITING_COLLECTION
  );
}
...
/**
 * Returns true if the Paystack payment window is still open
 */
export function isPaymentWindowOpen(order) {
  if (!order?.createdAt && !order?.$createdAt) return false;
  if (order.payment_method !== 'paystack') return true; // COD has no window
  if (order.payment_status === 'paid') return false;
  if (order.payment_status === 'expired') return false;

  const created = new Date(order.createdAt || order.$createdAt).getTime();
  const expiresAt = created + PAYMENT_WINDOW_MINUTES * 60 * 1000;
  return Date.now() < expiresAt;
}

/**
 * Mark an order's payment as expired
 */
export async function markPaymentExpired(orderId) {
  return databases.updateDocument(
    appwriteConfig.databaseId,
    appwriteConfig.ordersCollectionId,
    orderId,
    {
      ...
      // Optional: also cancel so kitchen stays clean
      status: 'Cancelled',
    }
  );
}
